...
public class Wordifier {
	...
	public static HashMap<String, String> leftUnigrams = new HashMap<String, String>();
	public static HashMap<String, String> rightUnigrams = new HashMap<String, String>();
	...
	/*first we initialize our scanner so we can scan over the file and then initialize 
	 * our linked list to hold any characters in any text file. Using a try catch we 
	 * check if the file will open, if not the program shuts off.  */
	...
	public static HashSet<String> findNewWords( HashMap<String,Integer> bigramCounts, HashMap<String,Double> scores, int countThreshold, double probabilityThreshold ) {
		...
		for (Entry<String,Integer> e : bigramCounts.entrySet()) {
			int count = bigramCounts.get(e.getKey());
			double score = scores.get(e.getKey());
			
			if (count >= countThreshold && score >= probabilityThreshold) {
				newWords.add(e.getKey());
			}
		}
	...
	public static LinkedList<String> resegment( LinkedList<String> previousData, HashSet<String> newWords ) {
		...
		ListIterator<String> itr = previousData.listIterator();
	...
	public static void computeCounts(LinkedList<String> data, HashMap<String,Integer> bigramCounts ) {
		...
		while (itr.hasNext()) {
			String leftUnigram = itr.next();
			String rightUnigram = "";
			
			if (itr.hasNext()) {
				rightUnigram = itr.next();
				itr.previous();
				
				String bigramKey = leftUnigram + " " + rightUnigram;
				
				incrementHashMap(bigramCounts, bigramKey, 1);
				leftUnigrams.put(bigramKey, leftUnigram);
				rightUnigrams.put(bigramKey, rightUnigram);
			}
		}
		return;
	}
	...
	public static void convertCountsToProbabilities(HashMap<String,Integer> bigramCounts, HashMap<String,Double> bigramProbs, HashMap<String,Double> leftUnigramProbs, HashMap<String,Double> rightUnigramProbs ) {
		...
		HashMap<String,Integer> leftUnigramCounts = new HashMap<String,Integer>();
		HashMap<String,Integer> rightUnigramCounts = new HashMap<String,Integer>();
		
		for (Entry<String,Integer> e : bigramCounts.entrySet()) {
			totalBigrams += e.getValue();
			
			String left = leftUnigrams.get(e.getKey());
			String right = rightUnigrams.get(e.getKey());
			
			incrementHashMap(leftUnigramCounts, left, e.getValue());
			incrementHashMap(rightUnigramCounts, right, e.getValue());
			
//			bigramProbs.put(e.getKey(), bigramCounts.get(e.getKey())/totalBigrams);
//			leftUnigramProbs.put(left, leftUnigramCounts.get(left)/totalBigrams);
//			rightUnigramProbs.put(right, rightUnigramCounts.get(right)/totalBigrams);
		}
		
		for (Entry<String,Integer> e : bigramCounts.entrySet()) {
			bigramProbs.put(e.getKey(), e.getValue()/totalBigrams);	
		}
		for (Entry<String,Integer> e : leftUnigramCounts.entrySet()) {
			leftUnigramProbs.put(e.getKey(), e.getValue()/totalBigrams);	
		}
		for (Entry<String,Integer> e : rightUnigramCounts.entrySet()) {
			rightUnigramProbs.put(e.getKey(), e.getValue()/totalBigrams);	
		}
	}
	...
	public static HashMap<String,Double> getScores( HashMap<String,Double> bigramProbs, HashMap<String,Double> leftUnigramProbs, HashMap<String,Double> rightUnigramProbs ) {
		...
		for (Entry<String,Double> e : bigramProbs.entrySet()) {
			String bigram = e.getKey();
			double bigramProb = e.getValue();
			
			String left = leftUnigrams.get(bigram);
			String right = rightUnigrams.get(bigram);
			
			double leftUnigramProb = leftUnigramProbs.get(left);
			double rightUnigramProb = rightUnigramProbs.get(right);
			
			double score = bigramProb/Math.sqrt(leftUnigramProb*rightUnigramProb);
			scores.put(bigram, score); 						
		}
	...
	public static void printNumWordsDiscovered( HashMap<String,Integer> vocab, HashSet<String> dictionary ) {
		...
		double uniquePercentage = Math.round(((double)uniqueWords.size()/(double)dictionary.size())*100);
		double totalPercentage = ((double)totalActualWords/(double)totalWords)*100;
		
		System.out.println("Discovered " + uniqueWords.size() + " actual (unique) words out of " + dictionary.size() + " dictionary words (" + uniquePercentage + "%)");
		System.out.println("Discovered " + totalActualWords + " actual word tokens out of " + totalWords + " total tokens (" + totalPercentage + "%)");
	}
